...
1. **Anchor**：從步驟 4 產出的來源頁中萃取核心主張（題數依 profile，預設 3）。每題完整描述主張內容（不用「這個」「該主張」等代名詞），出 A/B/C/D 選擇題給使用者
2. **Defend**：對使用者選非 A 的主張追問理由（50-150 字依 profile）；若全選 A，挑最具爭議的一題追問「最強同意理由」
...
**暫存草稿**：使用者在任一階段明確說「先暫停」「等等回來」等，將進度寫入 `wiki/.pending/<filename>.md`，**步驟 9、10 仍不執行**——直到下次恢復完成。
...
#### 10. 追加 log.md
記錄 ingest 操作、影響的頁面列表。若步驟 8.5 執行了，**同時**追加 `reflect` op（兩筆紀錄同日先後排列）。reflect op 格式見 `references/conventions.md`。
