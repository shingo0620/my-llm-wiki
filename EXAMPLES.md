...
接續範例 1 的來源情境（已 ingest「Attention Is All You Need」進到步驟 8）。為示範完整三段式，本範例假設使用者改選「最強」內化深度（`ingest_challenge_enabled: true`，啟用 Challenge 階段），而非範例 1 的「中等」。步驟 8.5 思考關卡：
...
```
你對「Transformer 完全捨棄 RNN 與 CNN，僅靠 self-attention 與 feed-forward 層，在 WMT 2014 英德翻譯達到 28.4 BLEU、超越先前所有 SOTA 集成模型」選了 B；也對「Self-attention 透過 O(1) 個 sequential operation 連結任意兩個位置，對學習長距離依賴有根本優勢」選了 B。請說明：
...
整體標 `#stance:partial-agree`（依規則 5，無存疑題，主張 1、2 的部分同意拉動整體立場）。
